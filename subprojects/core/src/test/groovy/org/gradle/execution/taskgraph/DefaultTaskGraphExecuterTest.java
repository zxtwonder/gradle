...
public class DefaultTaskGraphExecuterTest {
    ...
    private Task brokenTask(String name, final RuntimeException failure, final Task... dependsOn) {
        ...
        final TaskOutputsInternal outputs = context.mock(DefaultTaskOutputs.class);
    ...
    private void setExpectations(final String name, final TaskInternal task, final TaskStateInternal state, final TaskOutputsInternal outputs, final TaskDestroyables destroys) {
        context.checking(new Expectations() {{
            ...
            allowing(outputs).finalizeOutputFiles();
